...
Dit dataproduct draagt bij aan de detectie van energie-intensieve apparaten. De epic bestaat uit twee fasen, een pilot fase en een landelijke fase. 
...
=== Landelijke fase
In de landelijke fase wordt het statistische model landelijk toegepast op alle (goedgekeurde) aansluitingen. Wellicht gaat er eerst een kleine, regionale testfase aan vooraf. Het resulteert in een tabel met voor elk energie-intensief apparaat (PV, EV, WP, BESS) het geschatte vermogen (in kW(p)) een bijbehorende betrouwbaarheid (Confidence). Deze informatie is _niet_ per EAN, maar geaggreerd op middenspanningsruimte ( MS/LS trafo) danwel postcode6 niveau:

.Voorbeeld eindresultaat geaggregeerd op trafo)
...
In de pilot fase is gekozen voor een decentraal scenario waarbij de verbruiksdata van de slimme meters van de 1.000 medewerkers van de drie grootste RNB's door de RNB's met elkaar gedeeld wordt.

=== Landelijke fase

Er is gekozen voor een centraal scenario waarbij de geregistreerde verbruiksdata van de slimme meters centraal beschikbaar wordt gesteld aan deelnemende RNB's.
